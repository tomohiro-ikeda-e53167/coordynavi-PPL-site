// ISO 3166-1 alpha-3。name 昇順（localeCompare）・code 一意。
// RAW は資料の掲載順（ほぼ ASCII 順）で保持し、実際のエクスポートは
// テストと同じ localeCompare 比較でソートすることで、アクセント付き
// 文字（例: Åland Islands）の並び順が環境ロケールに依存しないようにする。
const RAW = [
...
// エクスポート時にテストと同じ localeCompare でソートすることで、
// アクセント付き頭文字（Åland Islands 等）の並び順を保証する。
export const COUNTRIES = [...RAW].sort((a, b) => a.name.localeCompare(b.name));
